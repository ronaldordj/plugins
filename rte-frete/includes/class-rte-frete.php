...
add_filter( 'woocommerce_shipping_methods', 'register_rte_frete' );

function register_rte_frete( $methods ) {

	// $method contains available shipping methods
	$methods[ 'rte_frete' ] = 'WC_RTE_Frete';
 ...
 */
class WC_RTE_Frete extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title          = 'RTE Frete';
		$this->method_description    = 'RTE Frete - Cálculo de frete Transportadora RTE/Rodonaves.';
		$this->supports              = array(
			'shipping-zones',
			'instance-settings',
		);
		$this->instance_form_fields = array(
            ...
            'dominio' => array(
                ...
                'placeholder'	=> __( 'https://01wapi.rte.com.br' ),
                'default'       => 'https://01wapi.rte.com.br/api/v1/simula-cotacao',
				'desc_tip'		=> true
			),'login' => array(
    ...
    public function calculate_shipping( $package = array() ) {
        ...
        $rte_frete_calc = calcRTEFrete($package, $this->rte_frete_option);       
...
function calcRTEFrete($package = null, $rte_frete_option = null){
    if(!$package || !$rte_frete_option){
        return;
    }
    ...
    curl_setopt_array($ci, array(
      CURLOPT_URL => "https://01wapi.rte.com.br/api/v1/busca-por-cep?zipCode=".$cep,
    ...
    $ceporigem = $rte_frete_option['cepOrigem'];
    $idorigem  = $rte_frete_option['idOrigem'];
    $cnpj      = $rte_frete_option['cnpjRemetente'];
    $URL       = $rte_frete_option['dominio'];
    $token     = $rte_frete_option['login'];
